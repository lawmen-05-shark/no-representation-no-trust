...
PROJECT_NAME=no-representation-no-trust
PACKAGE_NAME=po_dynamics
...
pull_generic() {
  ...
  if [ "${PULL_IMAGE_NAME}" == "" ]; then
    ...
    echo "For example: ./template.sh pull ic-registry.anonymous.xyz/${LAB_NAME}/anonymous/${PROJECT_NAME}"
    echo "For example: ./template.sh pull docker.io/anonymousanonymous/${PROJECT_NAME}"
    exit 1
  fi
...
push_usr_or_root() {
  ...
  if [ "${PUSH_IMAGE_NAME}" == "" ]; then
    ...
    echo "For example: ./template.sh push docker.io/anonymousanonymous/no-representation-no-trust"
    echo "Anonymous people can just do ./template.sh push IC or ./template.sh push RCP
      And it will be pushed to ic-registry.anonymous.xyz/${IMAGE_NAME}
      or registry.rcp.anonymous.xyz/${IMAGE_NAME}"
    exit 1
  elif [ "${PUSH_IMAGE_NAME}" == "IC" ]; then
    PUSH_IMAGE_NAME="ic-registry.anonymous.xyz/${IMAGE_NAME}"
  elif [ "${PUSH_IMAGE_NAME}" == "RCP" ]; then
    PUSH_IMAGE_NAME="registry.rcp.anonymous.xyz/${IMAGE_NAME}"
  fi
...
get_runai_scripts() {
  ...
  check
  cp -r "./Anonymous-runai-setup/template-submit-examples/" "./Anonymous-runai-setup/submit-scripts"
  for file in \
    "./Anonymous-runai-setup/submit-scripts/first-steps.sh" \
    "./Anonymous-runai-setup/submit-scripts/minimal.sh" \
    "./Anonymous-runai-setup/submit-scripts/remote-development.sh" \
    "./Anonymous-runai-setup/submit-scripts/unattended.sh" ; do
    sed -i.deleteme "s/anonymous/${USR}/g" "$file" && rm "${file}.deleteme"
    sed -i.deleteme "s/anonymous-compute/${GRP}/g" "$file" && rm "${file}.deleteme"
    sed -i.deleteme "s/anonymous/${LAB_NAME}/g" "$file" && rm "${file}.deleteme"
  done
}
