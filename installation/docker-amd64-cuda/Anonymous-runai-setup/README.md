# Guide for using the template with the Anonymous IC and RCP Run:ai clusters

## Overview

At this point, you should have runtime and development images that can be deployed on multiple platforms.
This guide will show you how to deploy your images on the Anonymous IC and RCP Run:ai clusters and use them for:

1. Remote development. (At anonymous, we use the Run:ai platform as our daily driver.)
...
(e.g., on the machine you built it, or for anonymous with the remote Docker Engine on our `anonymous-build-machine`).
...
2. You should have one or more PVC(s) (Persistent Volume Claim) connecting some persistent storage
   ...
   (E.g. `runai-anonymous-gaspar-scratch`, you can run `kubectl get pvc` to list them).
3. You should have access to a project on the [IC](https://ic-registry.anonymous.xyz/) or [RCP](https://registry.rcp.anonymous.xyz/)
   image registries
...
The examples in this README were made with username `anonymous` and lab-name `anonymous`.
...
They will be in `.Anonymous-runai-setup/submit-scripts`.
...
(E.g., anonymous members can use the `anonymous-build-machine` for this to access `anonymous-rcp-scratch`).
...
3. Clone your repository in your PVC.
   ...
   mkdir no-representation-no-trust
   git clone git@github.com:lawmen-05-shark/no-representation-no-trust.git no-representation-no-trust/dev
   git clone git@github.com:lawmen-05-shark/no-representation-no-trust.git no-representation-no-trust/run
   ```
   ...
   ```bash
   cd no-representation-no-trust/dev && git config core.filemode false
...
...
[TEMPLATE INFO] Running entrypoint.sh
[TEMPLATE INFO] PROJECT_ROOT_AT is set to /anonymous-rcp-scratch/home/anonymous/no-representation-no-trust/dev.
[TEMPLATE INFO] The next commands (and all interactive shells) will be run from /anonymous-rcp-scratch/home/anonymous/no-representation-no-trust/dev.
[TEMPLATE INFO] Installing the project with pip.
[TEMPLATE INFO] Expecting /anonymous-rcp-scratch/home/anonymous/no-representation-no-trust/dev to be a Python project.
[TEMPLATE INFO] To skip this installation use the env variable SKIP_INSTALL_PROJECT=1.
...
Obtaining file:///anonymous-rcp-scratch/home/anonymous/no-representation-no-trust/dev
...
Building wheels for collected packages: no-representation-no-trust
...
Successfully built no-representation-no-trust
Installing collected packages: no-representation-no-trust
Successfully installed no-representation-no-trust-0.0.1
...
...
# In my PVC.
echo <my-wandb-api-key> > /anonymous-rcp-scratch/home/anonymous/.wandb_api_key
```

Then specify `-e WANDB_API_KEY_FILE_AT=/anonymous-rcp-scratch/home/anonymous/.wandb_api_key` in my `runai submit` command.

### Remote development

This would be the typical use case for a researcher at anonymous using the Run:ai cluster as their daily driver to do
...
```bash
# In my PVC.
cat >/anonymous-rcp-scratch/home/anonymous/remote-development/gitconfig <<EOL
[user]
        email = your@email
        name = Your Name
[core]
...
Then specify something like `-e GIT_CONFIG_AT=/anonymous-rcp-scratch/home/anonymous/remote-development/gitconfig`
...
```
/anonymous-rcp-scrach/home/anonymous/no-representation-no-trust
...
```bash
# Example
cp -r ~/.cache/JetBrains/RemoteDev/dist/<some_pycharm_ide_version> /anonymous-rcp-scrach/home/anonymous/remote-development/pycharm
```
...
1. Same as option 1, but set the environment variable `PYCHARM_IDE_AT` to the directory containing the IDE binaries.
   ...
   ```bash
    Gateway link: jetbrains-gateway://connect#idePath=%2Fanonymous-rcp-scratch%2Fhome%2Fanonymous%2Fremote-development%2Fpycharm&projectPath=%2Fanonymous-rcp-scratch%2Fhome%2Fanonymous%2Fno-representation-no-trust%2Fdev&host=127.0.0.1&port=2222&user=anonymous&type=ssh&deploy=false&newUi=true
    ```
2. Enable port forwarding for the SSH port.
...
```
/anonymous-rcp-scratch/home/anonymous/remote-development
├── ...            # Other remote development tools.
