...
class Args:
    ...
    wandb_project_name: str = "po-dynamics-cleanrl-collapse"
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        actor_hidden = self.actor(x)
        action_mean = self.actor_mean(actor_hidden)
        action_logstd = self.actor_logstd(actor_hidden)
        action_std = torch.exp(action_logstd.clamp(min=np.log(1e-9), max=np.log(1e9)))
